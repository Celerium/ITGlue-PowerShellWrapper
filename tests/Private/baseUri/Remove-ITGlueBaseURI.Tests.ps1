<#
    ...
    .EXAMPLE
        Invoke-Pester -Path .\Tests\Private\baseUri\Remove-ITGlueBaseURI.Tests.ps1

        Runs a pester test and outputs simple results

    .EXAMPLE
        Invoke-Pester -Path .\Tests\Private\baseUri\Remove-ITGlueBaseURI.Tests.ps1 -Output Detailed
...
Describe "Testing [ $commandName ] function with [ $pester_TestName ]" -Tag @('baseUri') {
    ...
    Context "[ $commandName ] testing function" {
        ...
        It "If the baseUri is already gone a warning should be thrown" {
            Add-ITGlueBaseURI
            Remove-ITGlueBaseURI
            Remove-ITGlueBaseURI -WarningAction SilentlyContinue -WarningVariable baseUriWarning
