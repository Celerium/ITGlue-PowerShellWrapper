<#
    ...
    .EXAMPLE
        Invoke-Pester -Path .\Tests\Private\ModuleSettings\Get-ITGlueModuleSetting.Tests.ps1

        Runs a pester test and outputs simple results

    .EXAMPLE
        Invoke-Pester -Path .\Tests\Private\ModuleSettings\Get-ITGlueModuleSetting.Tests.ps1 -Output Detailed
...
Describe "Testing [ $commandName ] function with [ $pester_TestName ]" -Tag @('moduleSettings') {
    ...
    Context "[ $commandName ] testing function" {
        ...
        It "No configuration should populate baseline variables" {
            Import-ITGlueModuleSetting -ITGlueConfigPath $invalidPath -ITGlueConfigFile 'invalid.psd1'

            (Get-Variable -Name ITGlue_Base_URI).Value | Should -Be $(Get-ITGlueBaseURI)
            (Get-Variable -Name ITGlue_JSON_Conversion_Depth).Value | Should -Not -BeNullOrEmpty
        }
        ...
        It "Saved configuration session should contain required variables" {
            ...
            Import-ITGlueModuleSetting -ITGlueConfigPath $exportPath

            (Get-Variable -Name ITGlue_Base_URI).Value | Should -Not -BeNullOrEmpty
            (Get-Variable -Name ITGlue_API_Key).Value | Should -Not -BeNullOrEmpty
            (Get-Variable -Name ITGlue_JSON_Conversion_Depth).Value | Should -Not -BeNullOrEmpty
        }
