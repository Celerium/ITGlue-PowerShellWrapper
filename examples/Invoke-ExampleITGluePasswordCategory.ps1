<#
    ...
    .DESCRIPTION
        The Invoke-ExampleITGluePasswordCategory script populates example
        data using the various methods available to an endpoint

        By default on the first run this script will create 3 new password
        categories in the ITGlue tenant.

        If a password category already exists then it name is updated

        As of 2024-09-28, the password category endpoint does not support
        deletion so you will have to manually delete the password
        categories from ITGlue
    ...
    .EXAMPLE
        .\Invoke-ExampleITGluePasswordCategory.ps1 -Verbose

        Checks for existing password catagories and either updates or creates new example password
        catagories
...
#Region     [ Prerequisites ]
    ...
    $StepNumber     = 1
    $ExampleName    = 'ExamplePasswordCategory'
...
#Region     [ Find Existing Data ]
    ...
    $CurrentPasswordCategories = (Get-ITGluePasswordCategory -AllResults).data | Where-Object {$_.attributes.name -like "$ExampleName*"}
    if ($CurrentPasswordCategories) {
        Write-Verbose " -       - $(Get-Date -Format MM-dd-HH:mm) - Found [ $(($CurrentPasswordCategories| Measure-Object).Count) ] existing password catagories"
    }
...
#Region     [ Example Code ]
    ...
    while($ExampleNumber -le $ExamplesToMake) {
        ...
        $ExistingPasswordCategory = $CurrentPasswordCategories | Where-Object {$_.attributes.name -like "$ExamplePasswordName*"}

        if ($ExistingPasswordCategory) {

            Write-Verbose " -       - $(Get-Date -Format MM-dd-HH:mm) - Updating password category [ $($ExistingPasswordCategory.attributes.name) | $($ExistingPasswordCategory.id) ]"

            #Simple password field updates
            $UpdatedPasswordHashTable = @{
                type        = 'password-categories'
                attributes = @{
                    name = "$ExamplePasswordName-Updated-$(Get-Date -Format 'yyyy-MM-dd-HH:mm:ss')"
                }
            }

            $CategoryReturn = Set-ITGluePasswordCategory -ID $ExistingPasswordCategory.id -Data $UpdatedPasswordHashTable
            $ExampleReturnData.Add($CategoryReturn) > $null

        }
        else {

            Write-Verbose " -       - $(Get-Date -Format MM-dd-HH:mm) - New password category [ $ExamplePasswordName ]"
            ...
            $CategoryReturn = New-ITGluePasswordCategory -Data $NewPasswordHashTable
            $ExampleReturnData.Add($CategoryReturn) > $null

        }

        #Clear hashtable's for the next loop
        $UpdatedPasswordHashTable   = $null
...
#EndRegion  [ Example Code ]
    ...
    Write-Warning " -       - $(Get-Date -Format MM-dd-HH:mm) - You will have to manually delete [ $(($ExampleReturnData.data | Measure-Object).Count) ] password catagories from ITGlue"
